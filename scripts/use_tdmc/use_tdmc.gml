...
function use_tdmc(
  _placeMeeting = function(_x, _y){
    return place_meeting(_x, _y, collisionPixelObj); //Replace objWall with your object 
  }, 
  _cornerSlip = 16, 
  _slipSpd = .5, 
  _catchupFactor = .5) {
    ...
    xSpdYSpd: function(_xSpd, _ySpd) {
      ...
      var _timeoutTimer = 0;
        ...
        if(abs(__iXSpdLeft) >= 1) {
		  ...
		  //show_debug_message(__iXSpdLeft)
		  
		  // checks if player is not one pixel away from wall; then lets them move
...
// checks all possibilities of where to move play, down 1, plus 1, etc
            _againstHori = _dir;
