...
function Alarm(_time, _function, _loop = false, _startTime = _time) constructor {
	...
	/// @func							restartNewTime(TIME, FUNCTION);
	/// @desc	//SELFMODIFICATION		Reset the Alarm to its new time.
	static restartNewTimeFunction = function(TIME = noone, FUNCTION = noone) {
		if TIME != noone time = TIME;
		if FUNCTION != noone func = FUNCTION;
	}
